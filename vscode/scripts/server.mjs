...
const BAZEL_ECLIPSE_LATEST_URL =
	'https://eclipseguru.github.io/bazel-eclipse/latest/p2-repository.zip';
...
async function downloadServer() {
	console.log(
		'Downloading latest Bazel JDT Language Server extension release...'
	);
	...
	try {
		...
		const filesToExtract = directory.files.filter(
			(file) =>
		...
		await Promise.all(
			filesToExtract.map(async (file) => {
				const destPath = path.join(
					'./server',
					jarNameWithoutVersion(file.path)
				);
		...
		console.log(
			'Successfully downloaded and extracted Bazel JDT Language Server extension'
		);
	} catch (error) {
...
async function buildServer() {
	...
	try {
		const jdtlsJars = await glob(
			`${serverDir}/releng/p2repository/target/repository/plugins/com.salesforce.bazel.eclipse.jdtls_*.jar`
		);
		if (jdtlsJars.length != 1) {
			console.log(
				'Building Bazel JDT Language Server extension from source...'
			);
		...
		const sources = await glob(
			`${serverDir}/releng/p2repository/target/repository/plugins/*.jar`
		);

		// filter the jars that are included in the package.json
		const jarsToCopy = sources.filter((source) =>
			jarIsIncludedInPackageJson(jarNameWithoutVersion(source))
		);
		if (jarsToCopy.length > 0) {
			// copy the jars to the server directory
			await Promise.all(
				jarsToCopy.map(async (source) => {
					return await fs.copy(
						source,
						path.join('./server', jarNameWithoutVersion(source))
					);
				})
			);
			console.log(
				'Successfully built and copied Bazel JDT Language Server extension'
			);
		} else {
...
async function main() {
	...
	switch (command) {
		...
		case 'build-or-download':
			...
			break;
		default:
			console.log('Usage: node server.js [download|build|build-or-download]');
			process.exit(1);
	}
}
